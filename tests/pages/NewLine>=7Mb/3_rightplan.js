describe('Right plan page - NewLine>=7Mb', function () {
  before(function () {
    console.log('Load Scenario...');
    cy.loadScenarioNewLine('NewLine>7Mb/rightplan','', 'rightplan');
    //cy.wait('@postCall');
  });
  context('When user navigates to rightplan page for a NewLine scenario', function(){

    it('should contain the header title', function(){
      cy.get('h1')
      .should('contain', `Ok, let's help you get the right plan`);
    });

    it('should have the radio buttons for devices options', function(){
      cy.get('.rprcRadioButtonContainer.rprc_users')
      .should('exist');
    });
    it('should have the radio buttons for activities', function(){
      cy.get('.rprcRadioButtonContainer.rprc_activities')
      .should('exist');
    });
    ...
    it('should have the - What does this mean? - link for the first question', function(){
      cy.get('form.ng-pristine.ng-valid >div:nth-of-type(1)')
      .should('exist');
    });

    it('should open up the - What does this mean? and have a correct text', function(){
      cy.get('form.ng-pristine.ng-valid >div:nth-of-type(1) a')
      .click();
      cy.get('.supporting.ng-isolate-scope.supporting--open:nth-of-type(1)')
      .should('contain', 'Your home’s broadband speed is shared amongst everyone’s connected devices, so the more devices you have, the higher the speed you’ll need to enjoy a great experience.');
    });

    it('should have the - What does this mean? - link for the second question', function(){
      cy.get('form.ng-pristine.ng-valid >div:nth-of-type(4)')
      .should('exist');
    });

    it('should open up the - What does this mean? and have a correct text', function(){
      cy.get('form.ng-pristine.ng-valid >div:nth-of-type(4) a')
      .click();
      cy.get('#content > rprc > form > div:nth-child(7)')
      .should('contain', 'Each online activity requires some of your home’s broadband speed to work. So if you have lots of devices, doing lots of things online, all at the same time, the higher the speed you’ll need to enjoy a great experience.');
    });
    ...
    describe('When click on get plan options without select any option', () => {
      ...
      it('should be displayed error messages', function(){
        cy.get('.form-errors.notification.ng-isolate-scope.notification--error')
        .should('contain','Please confirm how many devices you use at busy times').
    ...
    describe('When clicking on a device radio button', () => {
      ...
      devicesOptions.forEach(option => {
        it(('should be selected the radio button: ' + option), () => {
          cy.get(`.rprcRadioButtonContainer.rprc_users rprc-radio-button:nth-child(${option})`)
          .click();
        });
      });
      it('should have the last button selected now', function(){
        cy.get('.rprcRadioButtonContainer.rprc_users rprc-radio-button:last-child>div')
    ...
    describe('When tick on an activity option', () => {
      ...
      devicesOptions.forEach(option => {
        it(('Should select the activity:' + option), () => {
          cy.get(`.rprcRadioButtonContainer.rprc_activities rprc-radio-button:nth-child(${option})`)
          .click();
        });
      });
    });
